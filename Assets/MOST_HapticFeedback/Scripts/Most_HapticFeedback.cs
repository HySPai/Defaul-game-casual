...
namespace Solo.MOST_IN_ONE
{
    public static class MOST_HapticFeedback
    {
        ...
        public static async void GeneratePattern(CustomHapticPattern hapticPattern)
        {
            if (IsPlaying) return;
            ...
            {
                await Task.Delay((int)haptic.Delay);
            ...
            {
                del += (int)haptic.Delay;
                pattern.Add((int)haptic.Delay);
                amp.Add(0);
            ...
            catch (TaskCanceledException) { /* Debug.Log("Vibration cancelled!");*/ }
            await _activePattern;
        }
